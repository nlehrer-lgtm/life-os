# Messenger Cup 2026 — Project Context

## What This Is
The 16th Annual Messenger Cup — Messenger International's primary annual fundraising event. Raises ~$4 million per year to fund global gospel resource distribution across 100+ nations. ~300 business leaders attend as underwriters, participants, and sponsors.

**Dates:** July 22–25, 2026
**Venue:** The Broadmoor Resort & Spa, Colorado Springs, CO
**Most important night:** Thursday, July 23 — banquet and fundraiser auction

## 2026 Theme
**"PERSEVERE — A People of Perseverance"**
Scripture anchor: Revelation 3:8 — *"I have placed before you an open door that no one can shut."*
Core message: America's prophetic mandate to spread the gospel through active, opposed perseverance. Not nationalism — mission.

## Current Phase
Pre-production / Script approval. Script v2 drafted and internally reviewed. Awaiting Lisa Bevere's sign-off before production begins.

Open decisions: see `STATUS.md`

## Nathaniel's Role
Video Execution Lead. Translates creative direction into deliverables, calls the live show on fundraiser night.

## Key People
| Person | Role |
|--------|------|
| Lisa Bevere | Primary creative direction — emotional tone, final vision sign-off |
| Bryson | VP Communications — message alignment, script feedback |
| Kathryn | Communications support — attends Lisa meetings, clarity feedback |
| Nathaniel Lehrer | Video Execution Lead |
| Andrew McIntosh | Editor/Director — owns edit from script through final delivery |
| Lindy Wood | On-site floor manager — run-of-show point person |
| Joel Yankee | Local Colorado Springs contractor — fundraiser night camera capture |

Full people notes: `people/andrew-mcintosh.md`, `people/lindy-wood.md`

## Centerpiece Deliverable: Opener Video
- **Runtime:** 1.5–2.5 minutes
- **When it plays:** Thursday night, July 23, after dinner, immediately before the fundraiser auction
- **Emotional goal:** Move the room deeply before paddles go up

**Concept:** One visual motif — a door. People from different nations press through persecution and opposition toward the same ancient wooden door. The knocking becomes music. At the climax, the door opens slightly and warm light reveals the Broadmoor ballroom — paddles on every seat.

**Final screen text:** *"I have placed before you an open door that no one can shut." — Revelation 3:8 / The nations are knocking. Tonight — we step through.*

**Creative files:**
- Opener concept (master doc): `opener-concept.md`
- Current script (v2): `opener-script-v2.md`
- Earlier script version: `opener-script-v1.md`
- Lisa's creative directives from Apr 8 meeting: `lisa-creative-notes.md`
- Brand guide: `brand-guide.md`
- Event logistics: `event-info.md`
- Event brief: `brief.md`

## Timeline
| Milestone | Date |
|-----------|------|
| Creative direction from Lisa | Apr 8, 2026 (done) |
| Script v2 sign-off from Lisa | Before ~May 6 (Lisa leaves for Italy) |
| Andrew tests AI motion transfer | Before ~May 6 |
| Film & capture | Mid-May to mid-June |
| Edit | Mid-June to mid-July |
| Hard lock | ~July 8, 2026 |
| Color grade / audio polish | July 8–18 |
| Event | July 22–25, 2026 |

## Music Direction
Primary: "Kiss the Ring" (found by Josh Kendrick) — slow cinematic build, international instrumentation, strong chime elements. The knocking in the video gradually becomes percussion syncing with the score. Lighting programmer syncs via Ableton to final video.
